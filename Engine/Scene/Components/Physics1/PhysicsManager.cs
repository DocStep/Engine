...
public static class PhysicsManager {
        ...
        foreach (List<ColliderComponent> bucket in grid.Values) {
            for (int i = 0; i < bucket.Count; i++) {
                for (int j = i + 1; j < bucket.Count; j++) {
                    ...
                    if (a.Overlaps(b, out Contact1 contact)) Resolve(a, b, contact);
                }
            }
        }
    }
    ...
    private static void ResolveAgainstStatic () {
        foreach (ColliderComponent dyn in dynamicColliders) {
            ...
            foreach (ColliderComponent stat in staticColliders) {
                ...
                if (dyn.Overlaps(stat, out Contact1 contact)) Resolve(dyn, stat, contact);
            }
        }
    }
    ...
    private static void Resolve (ColliderComponent a, ColliderComponent b, Contact1 contact) {
